...
export const resendInvitation = api(
  { method: "POST", path: "/household/resend-invitation", expose: true, auth: true },
  async (req: ResendInvitationRequest): Promise<void> => {
    ...
    if (householdResult?.owner_id !== auth.userID) {
      throw new Error("Only household owner can resend invitations");
    }
