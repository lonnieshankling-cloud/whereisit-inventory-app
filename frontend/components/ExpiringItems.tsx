...
import { AlertTriangle, Trash2, Move, CheckCircle } from "lucide-react";
...
export function ExpiringItems() {
  ...
  const fetchItems = useCallback(async (limit: number, offset: number) => {
    return await backend.item.expiring({ limit, offset });
  }, [backend]);
  ...
  if (isLoading) {
    return (
      <div className="bg-[#FEF3C7] rounded-lg p-6">
        <Accordion type="single" collapsible defaultValue="expiring">
          <AccordionItem value="expiring" className="border-none">
            <AccordionTrigger className="hover:no-underline">
              <div className="flex items-center gap-2">
                <AlertTriangle className="h-5 w-5 text-amber-600" />
                <span className="text-xl font-semibold text-[#111827]">Expiring Soon</span>
              </div>
            </AccordionTrigger>
            <AccordionContent>
      ...
      <Accordion type="single" collapsible defaultValue="expiring">
        <AccordionItem value="expiring" className="border-none">
          ...
          <AccordionContent>
            ...
            {items.length === 0 ? (
              <p className="text-gray-600">No items are expiring soon.</p>
            ) : (
              <>
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {items.map((item: any) => (
